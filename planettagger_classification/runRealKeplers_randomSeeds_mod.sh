...
for num_labels in `seq $nstart $nend`; do
  for seed in `seq $sstart $send`; do
    model_file=`echo "./realPlanets_mod/"$num_labels"classes/realKOIs_modFeatures_"$num_labels"classes_seed="$seed`
    results_path=`echo "./realPlanets_mod/"$num_labels"classes/"`
    echo $model_file
    echo $results_path
