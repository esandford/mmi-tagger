...
data_file="./realPlanets_incJ_final/realKOIs_finalFeatures_incJ_70percent.txt"
CVdata_file="./realPlanets_incJ_final/realKOIs_finalFeatures_incJ_30percent.txt"

num_planet_features=2
num_stellar_features=2
feature_names="[log10Rp,log10P,Teff,logg]"
batch_size=100
epochs=500
width=2
dropout_prob=0.01
...
for num_labels in `seq $nstart $nend`; do
  for seed in `seq $sstart $send`; do
    model_file=`echo "./realPlanets_incJ_final/"$num_labels"classes/realKOIs_finalFeatures_incJ_"$num_labels"classes_seed="$seed`
    results_path=`echo "./realPlanets_incJ_final/"$num_labels"classes/"`
    echo $model_file
    echo $results_path
