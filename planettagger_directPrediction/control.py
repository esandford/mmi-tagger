...
class Control(nn.Module):
    ...
    def classify(self, data_path, data):
        ...
        # and context data
        all_target_probs = np.zeros((1,self.model.num_planet_features))
        all_context_probs = np.zeros((1,self.model.num_planet_features))
        ...
        with torch.no_grad():
            for batch in batches:
                ...
                loss = nn.MSELoss()
                loss_actual = loss(context_probs, target_probs)
                
                avg_loss_actual += loss_actual 
        ...
        np.save("{0}{1}_truths_fromtarget_{2}.npy".format(self.results_path,data_path.split("/")[-1][:-4],self.seed),all_target_probs)
        
        # save class probabilities from context network (log-softmax form)
        np.save("{0}{1}_predictions_fromcontext_{2}.npy".format(self.results_path,data_path.split("/")[-1][:-4],self.seed),all_context_probs)
